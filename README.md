...
**int8 quantization — battle log (2026-09-06/09, `scripts/quantize_opset_convert.py`)**: the first two attempts (direct QOperator / QDQ on the opset-12 export) were both rejected by the task-level gate — detections dropped 41→0 (outputs collapsed to zero). Layer-by-layer dissection found the true culprit: **the per-tensor quantization scale on the output decode tail shared one scale between box coordinates (0–640) and class confidence (0–0.78), crushing the class channel to exactly zero** (the box channel looked fine only because its values are large). Fix: mechanical opset 12→16 conversion (unlocks QDQ) + excluding the class head and the three decode-tail nodes from quantization. Final **QOperator int8 passes the task-level gate**: detections 41→42, mean IoU 0.907, inference 58→37 ms (**1.57× faster**), size 42.7→15.2 MB (**2.8× smaller**); the app loader prefers int8 and falls back to fp32 automatically (on-device wasm fps to be measured). DINOv2 int8 remains blocked by quantizer operator compatibility and stays fp32 (it only runs on confirmed targets, so its cost is already bounded).
