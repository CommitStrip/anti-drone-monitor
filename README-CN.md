...
**int8 量化攻坚实录（2026-09-06/09，`scripts/quantize_opset_convert.py`）**：初始两次尝试（opset12 直接 QOperator / QDQ）均被任务级闸门拦截——检出 41→0（输出恒零）。逐层解剖确诊真凶：**输出解码尾部的 per-tensor 量化尺度把 0-640 的框坐标与 0-0.78 的类置信共享同一 scale，类通道被碾压成恒零**（框通道数值大所以"看似正常"）。修法：opset 12→16 机械化转换解除 QDQ 的 opset 封锁 + 类头与解码尾部三节点排除量化。最终 **QOperator int8 通过任务级闸门**：检出 41→42、IoU 均值 0.907、推理 58→37ms（**1.57×**）、体积 42.7→15.2MB（**2.8×↓**）；应用加载器 int8 优先、fp32 自动回退（wasm 实测帧率待回填）。DINOv2 int8 受量化器算子兼容性限制保持 fp32（只对确认目标运行，开销已可控）。
